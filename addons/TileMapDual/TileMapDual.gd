...
@export var inner_material: Material:
	set(value):
		# if inner_material != null and is_instance_valid(inner_material):
		# 	inner_material.changed.disconnect(_calc_handle_material_changed)
		inner_material = value
		# inner_material.changed.connect(_calc_handle_material_changed)
		if _display != null and is_instance_valid(_display):
			_display.calc_update_material(inner_material)
@export var outer_material: Material

# func _calc_handle_material_changed() -> void:
	# if _display != null and is_instance_valid(_display):
	# 	_display.calc_update_material(inner_material)
	# print("mateiral changed!")
...
func _ready() -> void:
	...
	# 	_display.owner = get_tree().edited_scene_root
	_display.calc_update_material(inner_material)
