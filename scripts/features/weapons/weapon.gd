...
class FireParams:
    ...
    func _init(pos: Vector2, d: Vector2, cp: float, c: float, col: int) -> void:
        self.position = pos
        self.direction = d
        self.charge_percentage = cp
        self.charge = c
        self.collision_layers = col
...
func fire(position: Vector2, direction: Vector2) -> bool:
    if _fire_rate_timer > 0.0:
        return false
    var params := FireParams.new(position, direction, get_charge_percentage(), _charge_timer, collision_layers)
