...
func repeat_raycast(from: Vector2, to: Vector2, check_callback: Callable, mask: int = 4294967295) -> Dictionary:
    var exclude: Array[RID] = [_get_node_rid(self.weapon_owner)]
...
func _get_node_rid(node: Node2D) -> RID:
    if "get_rid" in node:
        return node.get_rid()
    for n: CollisionShape2D in node.find_children("*", "CollisionShape2D"):
        return n.shape.get_rid()
    assert(false, "ERROR: Could not get RID for given node.")
    return RID() # impossible to reach so it's fine

func raycast(from: Vector2, to: Vector2, mask: int = 4294967295, exclude: Array[RID] = []) -> Dictionary:
    assert("get_world_2d" in self)
    var state: PhysicsDirectSpaceState2D = self.get("get_world_2d").call().direct_space_state
